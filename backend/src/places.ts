...
export function geocodeBlacksburg(location: string): Place {
  ...
  const hit = BLACKSBURG_PLACES.find(
    (place) => needle.includes(place.label.toLowerCase()) || place.label.toLowerCase().includes(needle)
  );
  if (hit) return hit;
  ...
  const scored = BLACKSBURG_PLACES.map((place) => {
    ...
    const score = tokens.reduce((sum, token) => (token.length > 2 && label.includes(token) ? sum + 1 : sum), 0);
    return { place, score };
...
export function pointToLatLng(location: { type: "Point"; coordinates: [number, number] }): { lat: number; lng: number } {
