  ...
  return (
    <div className="space-y-6" id="responses-view-container">
      ...
      <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-4" id="responses-kpi-row">
        <div className="rounded-2xl border border-[#382721] bg-[#231a17] p-4">
          <div className="mb-1 flex items-center justify-between text-xs text-[#9b8b84]">
            <span>Responses In</span>
            <Users className="h-4 w-4 text-[#b8744b]" />
          </div>
      ...
      <div className="flex flex-col items-stretch justify-between gap-3 rounded-2xl border border-[#33221c] bg-[#201512] p-3.5 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="pointer-events-none absolute left-3.5 top-3 h-4 w-4 text-[#786a63]" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search by guest token or rule text (e.g. peanuts, kosher, vegan)..."
            className="w-full rounded-xl border border-[#3a2822] bg-[#170f0e] py-2 pl-10 pr-4 text-xs text-white placeholder-[#786a63] transition-colors focus:border-[#b8744b] focus:outline-none"
          />
        </div>
          ...
          <button
            ...
            className={`whitespace-nowrap rounded-lg border px-3 py-1.5 font-medium transition-all ${
              activeCategory === 'all'
                ...
                : 'border-[#31201b] bg-[#180f0d] text-[#8e7e78] hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg border px-3 py-1.5 font-medium transition-all ${
              activeCategory === 'zero-match'
                ...
                : 'border-[#31201b] bg-[#180f0d] text-[#8e7e78] hover:text-[#f87171]'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg border px-3 py-1.5 font-medium transition-all ${
              activeCategory === 'high'
                ...
                : 'border-[#31201b] bg-[#180f0d] text-[#8e7e78] hover:text-[#fbbf24]'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg border px-3 py-1.5 font-medium transition-all ${
              activeCategory === 'contact'
                ...
                : 'border-[#31201b] bg-[#180f0d] text-[#8e7e78] hover:text-[#7dd3fc]'
            }`}
          >
      ...
      {/* Responses Table */}
      <div
        className="overflow-hidden rounded-2xl border border-[#352520] bg-[#231a17] shadow-xl"
        id="responses-table-card"
      >
        <div className="flex items-center justify-between border-b border-[#31211c] bg-[#1d1311] px-6 py-4">
          <div>
            <h3 className="text-base font-bold tracking-tight text-white">Anonymous Guest Responses</h3>
            <p className="mt-0.5 text-xs text-[#9b8b84]">
          ...
          <span className="rounded-full border border-[#3d2a23] bg-[#2a1d19] px-3 py-1 text-xs font-semibold text-[#a0918a]">
            Showing {filtered.length} of {responses.length}
          </span>
        </div>

        {responses.length === 0 ? (
          <div className="px-6 py-10 text-center text-sm text-[#9b8b84]">
              ...
              <tbody className="divide-y divide-[#2a1d19] text-xs">
                {filtered.map((response) => (
                  <tr
                    ...
                    className="group cursor-pointer transition-colors hover:bg-[#2c1d18]"
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-semibold text-white group-hover:text-[#f4ba95]">
                          {response.token}
                        </span>
                        <span
                    ...
                    <td className="max-w-[240px] px-4 py-4">
                      {response.hardExcludes.length > 0 ? (
                        ...
                        <span className="text-[11px] italic text-[#887872]">None reported</span>
                      )}
                    </td>

                    <td className="max-w-[220px] px-4 py-4">
                      <div className="truncate text-[11.5px] text-[#ddd2cd]" title={response.softPreferences.join(', ')}>
                        {response.softPreferences[0] || 'No stated preference'}
                      </div>
                      {response.softPreferences[1] && (
                        <div className="mt-0.5 truncate text-[11px] text-[#9b8b84]">
                          {response.softPreferences[1]}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {response.contactEmail ? (
                        ...
                        <span className="text-[11px] italic text-[#6d5e58]">Not provided</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg border border-[#3f2b23] bg-[#271916] px-3 py-1 text-xs font-semibold text-[#c9b9b3] transition-all group-hover:border-[#b8744b] group-hover:bg-[#b8744b] group-hover:text-white"
                      >
                        <span>View</span>
